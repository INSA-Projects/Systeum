...
int id = rand()%allocated_pages_capacity; 
printf("Now look at the big baby : %d",id);

//Trouve la page virtuelle associée (update allocated page)

int pg_v = allocated_pages[id];

// Vérifie si modification de la page (if p isDirty then swapwrite et cleardirty)

int modif = mmu_isDirty(pg_v);

// Désalloue la page physique (clear accessed invalidate page p)

pm_freePage(id);

// Invalide la traduction (pm_freepage)

if (modif==0) {
	mmu_invalidatePage(pg_v);
}

// Mémorise la page libére (page table set invalid swapped)
